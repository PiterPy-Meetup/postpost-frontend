...
storiesOf('PublicationItemPlatformIcon', module)
  ...
  .addDecorator(withDocs(PublicationItemPlatformIcon.options.__docs))
  .add('Just Icon', () => {
  ...
  .add("Icon within card", () => {
    return {
      ...
      template: `<v-layout justify-center>
  <v-flex xs6>
    <v-card>
  ...
  .addDecorator(withDocs(PublicationItemPlatformIcon.options.__docs))
  .add('Icon with null "avatar" property', () => {
    const networkIcons = SOCIAL_NETWORK_ICONS.slice();
    return {
      components: {PublicationItemPlatformIcon},
      props: {
